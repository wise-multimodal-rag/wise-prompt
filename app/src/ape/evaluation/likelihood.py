...
from app.src.ape import llm, data, evaluate
...
def get_query(prompt, eval_template, input_, output_, demo_data, demos_template):
    ...
    query = eval_template.fill(prompt=prompt, input=input_, output=output_, full_demo=demos)
    query_without_output = eval_template.fill(prompt=prompt, input=input_, output=special_output_token, full_demo=demos)
...
def likelihood_evaluator(prompts, eval_template, eval_data, demos_template, few_shot_data, config):
    ...
    for prompt in prompts:
        ...
        for d in zip(*subsampled_data):
            ...
            query, output_idx = get_query(prompt, eval_template, input_, output_, demo_data, demos_template)
...
class LikelihoodEvaluationResult(evaluate.EvaluationResult):
    ...
    def __init__(self, prompts, log_probs, num_samples):
        ...
        self.prompt_log_probs = self._compute_avg_likelihood(prompts, log_probs, num_samples)

    def _compute_avg_likelihood(self, prompts, log_probs, num_samples):
        prompt_log_probs = []  # type: ignore
